package ai.shreds.application; 
 ...
 public class ApplicationEnrollClientService implements ApplicationEnrollClientInputPort { 
     ...
     public AdapterEnrollClientResponse enrollClient(AdapterEnrollClientRequest request) { 
         ...
         if (clientRepository.findById(request.getClientId()) == null) { 
             throw new ApplicationValidationException("Client does not exist."); 
         } 
  
         // Map request to domain entity 
         DomainClientEntity clientEntity = loyaltyProgramMapper.mapToDomainEntity(request); 
  
         // Enroll client in loyalty program 
         try { 
             loyaltyProgramManagerService.enrollClient(clientEntity.getClientId(), clientEntity.getLoyaltyProgramId()); 
     ...
     private void validateRequest(AdapterEnrollClientRequest request) { 
         ...
         if (request.getEnrollmentDate().isAfter(java.time.LocalDateTime.now())) { 
